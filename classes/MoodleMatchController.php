...
class MoodleMatchController extends IvsMatchControllerBase implements IIvsMatch {
    ...
    public function handle_request($endpoint, $patharguments, $method, $postdata) {
        ...
        $response = parent::handle_request($endpoint, $patharguments, $method, $postdata); // TODO: Change the autogenerated stub
     ...
     */
    public function match_questions_get_by_video_db_order($videoid, $order = 'timecode', $skip_access = false) {
        global $DB;
        ...
        foreach ($reportinganswers as $key => $answer) {
            ...
            if ($answer['answers'][1]['user_id'] === $user->id) {
                ...
                if ($numanswers > 0) {
                    ...
                    $singlechoicedata['last'] = !empty($last_answer['is_correct']) ? 1 : 0;
    ...
    public function get_question_summary_data($question, $coursestudents) {
        ...
        $questionsummary->questionid = $question['nid'];
        $questionsummary->questiontitle = $question['title'];
        $questionsummary->questionbody = $question['question_body'];
        $questionsummary->numstudentstotal = count($coursestudents);
        $questionsummary->numstudentsparticipation = 0;
        $questionsummary->firstattemptcorrect = 0;
        $questionsummary->lastattemptcorrect = 0;
        ...
        foreach ($useranswerdata as $answer_data) {
            if ($answer_data['first_attempt_correct']) {
                $questionsummary->firstattemptcorrect++;
            }
            if ($answer_data['last_attempt_correct']) {
                $questionsummary->lastattemptcorrect++;
            }
        }

        $questionsummary->numstudentsparticipation = count($useranswerdata);
    ...
    public function get_question_summary_formated($question, $coursestudents) {
        ...
        $data->question_id = $questionsummary->questionid;
        $data->question_title = $questionsummary->questiontitle;
        $data->question_body = $questionsummary->questionbody;
        ...
        switch ($questionsummary->question_type) {
            ...
            case 'click_question':
                ...
                $data->question_first_try = $questionsummary->numstudentsparticipation == 0 ? '0%' :
                        round($questionsummary->firstattemptcorrect * 100 / $questionsummary->numstudentsparticipation, 0) .
                        '%';
                $data->question_last_try = $questionsummary->numstudentsparticipation == 0 ? '0%' :
                        round($questionsummary->lastattemptcorrect * 100 / $questionsummary->numstudentsparticipation, 0) .
                        '%';
                break;
        ...
        $data->question_answered = $questionsummary->numstudentsparticipation . ' / ' . $questionsummary->numstudentstotal;
